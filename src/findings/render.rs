...
/// for the reader's attention when the interesting number is almost always a
/// lane.
///
...
pub fn cost_table(total: &Usage, total_models: &[String], lanes: &[(String, Usage, Vec<String>)]) -> String {
    ...
    // Nothing but a total means there is no breakdown to align, and a
    // one-row table reads worse than the sentence it replaced.
    if rows.len() < 2 {
...
fn row(label: &str, usage: &Usage, models: &[String]) -> [String; 6] {
    let cached = if usage.input_tokens > 0 {
        ...
        // Not "0 cached (0%)": nothing was sent, so there was nothing to hit
        // the cache, and a zero here reads as a cache that missed.
        String::new()
    };

    let mut models = models.join(", ");
    if usage.embed_tokens > 0 {
        // Embedding spend rides in the model column rather than a column of its
        // own, which would be empty on every row of a review that never touched
        // the index.
        let embedded = format!("{} embedded", thousands(usage.embed_tokens));
        models = if models.is_empty() {
            embedded
        } else {
            format!("{models} · {embedded}")
        };
    }
