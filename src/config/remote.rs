...
//!   `preview.branch`, which decide where every published screenshot is
//!   served from — a repository that could set the first would have the bot
//!   embed pictures from a host of its choosing, and one that could set the
//!   second would have the App commit files onto a branch of its choosing.
//!   `preview.enabled` and `preview.max_flows` are
