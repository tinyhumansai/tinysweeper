...
#[test]
fn scratch_print_the_comment() {
    let diffs = [
        diff("src/lanes/critique.rs", 42),
        diff("src/lanes/security.rs", 9),
        diff("src/graph/build.rs", 14),
        diff("docs/modules/graph/README.md", 6),
    ];
    let walk = Neighbourhood {
        nodes: vec![
            file("src/lanes/critique.rs"),
        ...
        edges: vec![
            ...
            edge("src/retrieve/expand.rs", "src/graph/build.rs", EdgeKind::Calls),
    ...
    let map = build(
        ...
        &[
            finding("src/lanes/critique.rs", Severity::High),
            finding("src/graph/build.rs", Severity::Low),
        ],
        GraphView::Walked(&walk),
        &limits(),
    );
    println!("{}", comment(&map).unwrap());
}
