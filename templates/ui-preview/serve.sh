...
set -euo pipefail
test -d "$TS_CHECKOUT" || { echo "TS_CHECKOUT is not a directory: $TS_CHECKOUT" >&2; exit 1; }
cd "$TS_CHECKOUT"
...
# A stalled install or build (network hiccup, a corrupted lockfile) would
# otherwise block forever and leak the job; cap it instead of trusting the
# caller's own timeout alone.
